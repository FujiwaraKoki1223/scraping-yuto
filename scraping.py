...
def test_get_levels():
    for yuto_URL in yuto_URLs:
        ...
        html_to_flat_array(jobs, test)
        levels.append(get_level(test))

        all_statuses = {}
        key = False
        n = 0
        status_data = yuto.find_all(id="sub-status").text
        ...
        for stt,add in zip(["dex", "agi", "str", "vit", "int", "mnd"], ["A", "B", "C", "D", "E", "F"]):
            stt_data = yuto.find_all(id=f"stt-{stt}")
            stt_value = int(stt_data.find_all("dd").text)
            add_data = yuto.find_all(id=f"stt-add-{add}")
            add_value_txt = add_data.find_all("dd").text
